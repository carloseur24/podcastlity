...
def run_file_picker(purpose: str, exclude: Optional[str] = None) -> Optional[str]:
    ...
    while True:
        ...
        if choice == "0":
            ...
            if not videos:
                if exclude:
                    menus.print_warning(f"Solo hay un video (usado para {purpose}), usa Explorar o Introducir ruta")
                else:
            ...
            while True:
                ...
                menus.console.print("[dim]Comandos: ↑ = parent | Enter = seleccionar archivo | q = salir[/dim]")
                menus.console.print()
...
def run_continue_session() -> None:
    ...
    while True:
        ...
        menus.console.print(f"Perfil: {session.profile or 'longform'} | Tema: {session.topic}")
        menus.console.print()
...
def run_single_stage(session_id: str) -> None:
    ...
    choice = menus.Prompt.ask("\n[bold]Selecciona etapa[/bold]", choices=["0"] + [str(i) for i in range(1, len(stages)+1)])
...
def run_settings() -> None:
    ...
    if choice == "0":
        ...
        new_path = menus.Prompt.ask("Nuevo directorio de grabaciones", default=settings.get("recordings_mount", ""))
        settings["recordings_mount"] = new_path
    elif choice == "2":
        new_model = menus.Prompt.ask("Nuevo modelo Whisper", default=settings.get("whisper_model", "large-v2"))
        settings["whisper_model"] = new_model
    elif choice == "3":
        new_res = menus.Prompt.ask("Nueva resolucion proxy (ej: 1280x720)", default=settings.get("proxy_resolution", "1280x720"))
