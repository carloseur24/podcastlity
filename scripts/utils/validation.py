...
class SessionIdValidator:
    ...
    def validate(cls, value: str) -> ValidationResult:
        ...
        return ValidationResult(
            False, error="Solo letras, numeros, guiones y underscores (ej: mi-video-001)"
        )
...
class DurationValidator:
    ...
    def validate(cls, value: str) -> ValidationResult:
        ...
        if duration > cls.MAX_MINUTES:
            return ValidationResult(False, error=f"Maximo {cls.MAX_MINUTES} minutos")
...
class TopicValidator:
    ...
    def validate(cls, value: str) -> ValidationResult:
        ...
        if len(value) < cls.MIN_LENGTH:
            return ValidationResult(
                False, error=f"Tema muy corto (min {cls.MIN_LENGTH} caracteres)"
            )

        if len(value) > cls.MAX_LENGTH:
            return ValidationResult(
                False, error=f"Tema muy largo (max {cls.MAX_LENGTH} caracteres)"
            )
...
class FilePathValidator:
    ...
    def validate(cls, value: str) -> ValidationResult:
        ...
        if not any(path.lower().endswith(ext) for ext in cls.VALID_EXTENSIONS):
            return ValidationResult(
                False, error=f"Extension no valida. Usa: {', '.join(cls.VALID_EXTENSIONS)}"
            )
