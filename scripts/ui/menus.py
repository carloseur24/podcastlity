...
def print_session_info(session) -> None:
    ...
    table.add_row("Sesion:", f"[bold]{session.session_id}[/bold]")
    ...
    console.print(Panel(table, title="[bold]Info de Sesion[/bold]", border_style="blue"))
...
def prompt_session_id(default: str = "", max_retries: int = 3) -> str:
    ...
    for attempt in range(max_retries):
        value = Prompt.ask(
            "[bold cyan]ID de sesion[/bold cyan] (YYYYMMDD_slug)",
            default=default,
        )
        ...
        print_error(result.error or "Validacion fallida")
...
def prompt_topic(max_retries: int = 3) -> str:
    ...
    print_warning("Por favor ingresa un tema valido")
    return ""
...
def prompt_duration(default: float = 10.0, max_retries: int = 3) -> float:
    ...
    for attempt in range(max_retries):
        value = Prompt.ask(
            "[bold cyan]Duracion aproximada (minutos)[/bold cyan]",
            default=str(default),
        )
...
def prompt_tone() -> str:
    ...
    console.print("  1. [cyan]Directo y pratico[/cyan]")
    ...
    tones = {"1": "directo y pratico", "2": "amigable", "3": "profesional", "4": "casual"}
    return tones[choice]
...
def print_stage_menu(session_id: str, current_stage: str) -> None:
    ...
    table.add_row("2", "[cyan]Ejecutar etapa especifica[/cyan]")
    ...
    console.print(Panel(
        f"Sesion: [bold]{session_id}[/bold] | Stage: [yellow]{current_stage}[/yellow]",
        title="[bold]Continuar Sesion[/bold]",
        border_style="cyan",
    ))
    console.print(table)
...
def print_settings_menu(current_settings: dict) -> None:
    ...
    table.add_row("[cyan]Resolucion proxy:[/cyan]", current_settings.get("proxy_resolution", ""))
    
    console.print(Panel(table, title="[bold]Configuracion Actual[/bold]", border_style="cyan"))


def prompt_setting_change() -> str:
    console.print("\n[bold]Que deseas modificar?[/bold]")
    console.print("  1. [cyan]Directorio de grabaciones[/cyan]")
    console.print("  2. [cyan]Modelo Whisper[/cyan]")
    console.print("  3. [cyan]Resolucion proxy[/cyan]")
    console.print("  0. [dim]Volver[/dim]")
    
    return Prompt.ask(
        "\n[bold cyan]Selecciona opcion[/bold cyan]",
        choices=["0", "1", "2", "3"],
        default="0",
    )
