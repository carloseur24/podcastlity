...
class Pipeline:
    ...
    def _run_prepare(self, session_id: str) -> dict:
        """Run combined prepare stage for shorts (preprocess + analyze + cutmap)."""
        from scripts import audio_preprocess
        
        audio_preprocess.run(session_id, mode="shorts", workspace=self.workspace)
    ...
    def _is_stage_completed(self, stage_key: str, current_status: str, stages: list[StageInfo]) -> bool:
