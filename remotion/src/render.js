...
async function renderVideo(
  ...
  if (rangeStart !== null && rangeEnd !== null) {
    ...
    console.log(
      "[remotion]    Output: 0s to",
      actualDuration,
      "s (no frame offset, captions shifted)"
    );
    console.log("[remotion] ════════════════════════════════════════════════════");
  }
  ...
  if (rangeStart !== null && rangeEnd !== null) {
    ...
    console.log(
      "[remotion] Filtered & shifted captions:",
      captions.length,
      "/",
      allCaptions.length,
      "items"
    ...
    console.log(
      "[remotion] First caption after shift:",
      captions[0]?.text,
      "at",
      captions[0]?.startMs,
      "ms"
    ...
    console.log(
      "[remotion] Last caption after shift:",
      captions[captions.length - 1]?.text,
      "at",
      captions[captions.length - 1]?.startMs,
      "ms"
    );
  ...
  try {
    ...
    await renderMedia({
      ...
      onProgress: progress => {
        ...
        if (progress > 0) {
          console.log(
            `[remotion] Progress: ${(progress * 100).toFixed(1)}% | Elapsed: ${(elapsed / 1000).toFixed(1)}s | ETA: ${(eta / 1000).toFixed(1)}s`
          );
        }
      },
      // Start callback
...
if (require.main === module) {
  ...
  if (args.length < 8) {
    console.error(
      "Usage: node render.js <input> <output> <captions> <preset> <fps> <duration> <width> <height> [options]"
    );
    console.error("");
    ...
    console.error(
      "  Full render:           node render.js input.mp4 output.mp4 caps.json preset.json 30 57 1920 1080"
    );
    console.error(
      "  Proxy mode (480p):    node render.js input.mp4 output.mp4 caps.json preset.json 30 57 1920 1080 --proxy"
    );
    console.error(
      "  Proxy 144p (fast):    node render.js input.mp4 output.mp4 caps.json preset.json 30 57 1920 1080 --proxy --resolution 144p"
    );
    console.error(
      "  Range mode:           node render.js input.mp4 output.mp4 caps.json preset.json 30 57 1920 1080 --start 16 --end 48"
    );
    process.exit(1);
  }
