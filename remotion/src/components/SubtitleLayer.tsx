...
export const KineticSubtitle: React.FC<KineticSubtitleProps> = ({
  ...
  const currentTokenIndex = activePage.tokens.findIndex(
    token => token.fromMs <= currentTimeMs && currentTimeMs < token.toMs
  );
  ...
  console.log(
    "[SubtitleLayer] Active page:",
    activePage.text.substring(0, 20),
    "| Words:",
    displayTokens.map(t => t.text).join(" "),
    "at frame",
    adjustedFrame
  );
